...
namespace Cursos
{
    public partial class CreacionCursos : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("El límite de estudiantes debe ser entre 10 y 30.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            {
                MessageBox.Show("El código del curso ya está en uso, elija otro.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void GuardarCursoEnBaseDeDatos(string codigo, string nombreCurso, string grupo, int limiteEstudiantes)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    MessageBox.Show($"{recordsAffected} curso(s) {(isModifyMode ? "modificado(s)" : "agregado(s)")} en la base de datos.");
                    CargarCursos();
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Error al {(isModifyMode ? "modificar" : "guardar")} curso en la base de datos: " + ex.Message);
                }
            }
        }
        ...
        private void EliminarCursoEnBaseDeDatos(string codigoCurso)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    connection.Open();
                    int recordsAffected = command.ExecuteNonQuery();
                    MessageBox.Show($"Curso con código {codigoCurso} eliminado de la base de datos.");
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Error al eliminar curso con código {codigoCurso}: " + ex.Message);
                }
            }
        }
        ...
        private void EliminarCurso(string codigoCurso)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    connection.Open();
                    int recordsAffected = command.ExecuteNonQuery();
                    MessageBox.Show($"{recordsAffected} curso(s) eliminado(s) de la base de datos.");
                }
                catch (Exception ex)
                {
